...
async function handleMessageChange(value: any) {
  ...
  for (const message of messages) {
    ...
    if (type === "text" && text) {
      ...
      console.log(`[Webhook] Unsupported message from ${from}: errorCode=${err?.code}, errorTitle=${err?.title}, keys=${Object.keys(message).join(",")}`);

    } else {
      messageContent = `[Unsupported: Message type unknown]`;
      interactiveData = {
        type: "unsupported",
        originalType: type,
        messageKeys: Object.keys(message),
      };
      console.log(`[Webhook] Unknown message type "${type}" from ${from}, keys: ${Object.keys(message).join(",")}`);
    }
